...
def generate_code_from_description(code):
    ...
    User's code description:
    {code}
    
    **Important Note:** If you see code starting with ```python and ending with ``` then remove it.
    """
...
def critique_code(code):
    critique_prompt = f"""Analyze and critique the following Python code. 
        ...
        """
    try:
        response = ollama.generate(prompt=critique_prompt, model=model)
        return response.response
    except Exception as e:
